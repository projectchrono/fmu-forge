...
#include "FmuForgeUnitDefinitions.h"
...
class FmuVariableExport : public FmuVariable {
  public:
    ...
    void GetValue(fmi3VarType* varptr_ext, size_t nValues) const {
        if (is_pointer_variant(m_varbind)) {
            ...
            // copy the values from the bound variable to the external variable provided by the user
            for (size_t size = 0; size < nValues; size++) {
  ...
  protected:
    ...
    /// In case of arrays it concatenates the values with space delimitation.
...
/// - defining FMU variables (causality, variability, start value, etc)
...
class FmuComponentBase {
  public:
    ...
    /// Return the location of the FMU resources directory, as a file system path.
    ...
    fmi3Status fmi3GetVariable(const fmi3ValueReference vrs[], size_t nvr, T values[], size_t nValues) {
        ...
        //// and check if they match any of the requested variables
        size_t values_idx = 0;
  ...
  protected:
    ...
    /// - the FMU was instantiated with `loggingOn=fmi3True` and a msg_cat has been labeled as a debugging category;
